...
RUN apt-get update && \
    apt-get install -y cmake ffmpeg p7zip rar unrar libavdevice-dev libavfilter-dev libavformat-dev \
    libavcodec-dev libavutil-dev libswresample-dev libswscale-dev pkg-config && \
    rm -rf /var/lib/apt/lists/*

RUN pip install fastapi[standard]>=0.136.1 loguru>=0.7.3 numpy>=2.4.4 pydantic>=2.13.4 timecode>=1.5.1 uvicorn>=0.46.0 pillow>=12.2.0 puremagic>=2.2.0 patool>=4.0.5 --break-system-packages
